...
import { Panel, Position, type Node, type Edge, useReactFlow } from "@xyflow/react";
...
export function SequencerToolbar() {
  const { addNodes, screenToFlowPosition, getNodes, getEdges, setNodes, setEdges, fitView } = useReactFlow();
  ...
  return (
    <Panel position="bottom-left" style={{ left: "50%", transform: "translateX(-50%)", bottom: 16 }}>
      <div className="rounded-md border bg-card/80 backdrop-blur px-3 py-2 shadow-md">
        <div className="flex items-center gap-2">
          <Button type="button" variant="outline" className={`nodrag h-9 gap-2 ${borderClass}`} onClick={addBuyerImportAtCenter} aria-label="바이어 임포트 노드 추가" title="바이어 리스트 Import">
            <Upload className="size-4" />
          </Button>
          <Button type="button" variant="outline" className="nodrag h-9" onClick={() => runElkLayout("DOWN")} aria-label="세로 정렬" title="세로 정렬">
