...
export const customerGroupKeys = {
  ...
  list: (params?: CustomerGroupsParams) => [...customerGroupKeys.lists(), params] as const,
  detail: (id: string) => [...customerGroupKeys.all, "detail", id] as const,
  members: (groupId: string) => [...customerGroupKeys.all, "members", groupId] as const,
  membersList: (groupId: string, page?: number) =>
    [...customerGroupKeys.members(groupId), page] as const,
  workspace: (workspaceId: string) => [...customerGroupKeys.all, "workspace", workspaceId] as const,
  leadGroups: (leadId: string) => [...customerGroupKeys.all, "leadGroups", leadId] as const,
}
...
export function useCustomerGroupMembers(groupId: string, page = 1, limit = 10, enabled = true) {
  return useQuery({
...
export function useCustomerGroupsByWorkspace(workspaceId: string, enabled = true) {
  return useQuery({
...
export function useUpdateCustomerGroup() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: customerGroupKeys.detail(variables.groupId) })
      queryClient.invalidateQueries({ queryKey: customerGroupKeys.lists() })
...
export function useAddGroupMember() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: customerGroupKeys.members(variables.groupId) })
      queryClient.invalidateQueries({ queryKey: customerGroupKeys.detail(variables.groupId) })
...
export function useBulkAddGroupMembers() {
  ...
  return useMutation({
    mutationFn: (data: BulkAddMembersRequest) => customerGroupsApi.bulkAddMembers(data),
...
export function useBulkRemoveGroupMembers() {
  ...
  return useMutation({
    mutationFn: ({ groupId, leadIds }: { groupId: string; leadIds: string[] }) =>
