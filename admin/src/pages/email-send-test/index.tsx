...
export default function EmailSendTestPage() {
  ...
  const [groupMembers, setGroupMembers] = useState<
    Array<{ id: string; email: string; name?: string; primaryEmail?: string }>
  >([])
  ...
  const fetchGroupMembers = async (groupId: string) => {
    ...
    try {
      ...
      let result: {
        success: boolean
        data?: Array<{ id: string; email: string; name?: string; primaryEmail?: string }>
        message?: string
      }
      try {
      ...
      toast.error(
        `고객 그룹 멤버를 가져오는데 실패했습니다: ${error instanceof Error ? error.message : String(error)}`
      )
      setGroupMembers([])
    } finally {
...
`
  ...
  const handleGroupSend = async () => {
    ...
    // 이메일 주소가 있는 멤버만 필터링
    const membersWithEmail = groupMembers.filter((member) => member.primaryEmail)
            ...
            <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline">
                            이메일 주소 보유: {groupMembers.filter((m) => m.primaryEmail).length}명
                          </Badge>
                        </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="텍스트 형식의 이메일 본문"
                  value={groupBodyText}
                  onChange={(e) => setGroupBodyText(e.target.value)}
                  rows={6}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="<p>HTML 형식의 이메일 본문</p>"
                  value={groupBodyHtml}
                  onChange={(e) => setGroupBodyHtml(e.target.value)}
                  rows={6}
                />
              </div>
