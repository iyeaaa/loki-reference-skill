...
2. 이메일에는 반드시 다음 placeholder들을 적절히 활용하세요 (모든 placeholder를 사용할 필요는 없음):
   - {{회사명}}, {{웹사이트}}, {{업종}}, {{설명}}
   - {{국가}}, {{도시}}, {{주/도}}, {{주소}}
   - {{담당자명}}, {{이메일}}
   - {{직원수}}, {{설립연도}}
   - {{리드소스}}, {{리드상태}}, {{리드점수}}
3. placeholder는 실제 값으로 치환하지 말고 그대로 {{변수명}} 형태로 유지하세요
4. 전문적이면서도 친근한 톤을 유지하세요
5. 명확한 가치 제안과 행동 촉구(CTA)를 포함하세요
6. 비즈니스 이메일 형식에 맞춰 작성하세요
