...
async function main() {
  ...
  try {
    ...
    console.log(`  • Missing intent only: ${unclassifiedReplies.filter(r => !r.currentIntent && r.currentSentiment).length}`);
    console.log(`  • Missing sentiment only: ${unclassifiedReplies.filter(r => r.currentIntent && !r.currentSentiment).length}`);
    console.log(`  • Missing both: ${unclassifiedReplies.filter(r => !r.currentIntent && !r.currentSentiment).length}\n`);
