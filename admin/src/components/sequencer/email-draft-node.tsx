...
export const EmailDraftNode = memo(({ data, id }: { data: DraftData; id: string }) => {
  ...
  const generateAllDrafts = useCallback(async () => {
    ...
    try {
      ...
      setEmailDrafts((prev) => {
        const next = { ...prev } as Record<string, { subject: string; body: string }>
  ...
  return (
    <NodeStatusIndicator status={isBatchGenerating ? "loading" : "initial"} variant="border">
      <BaseNode className={`w-96 relative`}>
        <BaseNodeHeader className="border-b relative z-10">
          <div className="flex items-center gap-2">
            <FileText className="size-4" />
            <BaseNodeHeaderTitle>{data.title ?? "이메일 초안"}</BaseNodeHeaderTitle>
          </div>
          <Button
            variant="outline"
