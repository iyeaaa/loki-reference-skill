...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (attachmentCount > 0 || (files && files.length > 0)) {
			...
			if (body["attachment-info"]) {
				try {
					...
					Object.keys(attachInfo).forEach((key, idx, arr) => {
						...
						console.log(
							`${prefix} ${info.filename || key}: ${info.type} (${info["content-id"] || "no-id"})`,
						);
					});
				} catch {
			...
			if (files && files.length > 0) {
				...
				files.forEach((file: any, index: number) => {
					console.log(
						`   ${index + 1}. ${file.originalname || file.fieldname} (${file.size} bytes)`,
					);
				});
			}
		}
		...
		if (body.email) {
			...
			const contentEncodingMatch = body.email.match(
				/Content-Transfer-Encoding:\s*(base64|quoted-printable)/i,
			);
			if (
				contentEncodingMatch &&
				contentEncodingMatch[1].toLowerCase() === "base64"
			) {
				const base64BodyMatch = body.email.match(
		...
		if (
			body.to &&
			(body.to.includes("admin@grinda.ai") ||
				body.to.includes("rinda@partners.grinda.ai"))
		) {
			console.log("\n📮 [자동 답장 처리]");
			...
			if (!emailContent || emailContent.includes("�")) {
				emailContent =
					"(내용을 확인할 수 없습니다. 원본 이메일을 확인해 주세요.)";
			}
			...
			sendAutoReply(body.to, fromEmail, emailSubject, emailContent).then(
				(success) => {
					if (success) {
