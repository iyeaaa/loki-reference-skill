...
# 사용법 출력
usage() {
  echo -e "${BOLD}Usage:${NC} ./ci-server.sh [option]"
  echo ""
  ...
  echo "  ./ci-server.sh       # 전체 빌드"
  echo "  ./ci-server.sh fast  # 빠른 검사"
  echo "  ./ci-server.sh full  # 전체 빌드"
}
...
echo ""
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BOLD}  SendCI ${NC}${GRAY}v1.0${NC} ${BOLD}• Elysia-server (Backend)${NC}"
...
if [ "$MODE" = "fast" ]; then
  ...
  log_step "Running full build (lint + type-check + bun build)..."
  bun run build
  EXIT_CODE=$?
...
echo ""
if [ $EXIT_CODE -eq 0 ]; then
  echo -e "${GREEN}${BOLD}✓ Elysia-server checks passed${NC} ${GRAY}(${DURATION}s)${NC}"
  exit 0
else
  echo -e "${RED}${BOLD}✗ Elysia-server checks failed${NC} ${GRAY}(${DURATION}s)${NC}"
  exit 1
fi
