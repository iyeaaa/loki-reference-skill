                ...
                <h3 className="text-base font-semibold">
                  {isComplete
                    ...
                    : isError
                      ? "Lead Import Failed"
                      : isInit
                        ? "Preparing Lead Import"
                        : "Lead Import in Progress"}
                </h3>
              </div>
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-4 space-y-2">
              ...
              {progress.result.duration && (
                <div className="pt-2 border-t text-xs text-muted-foreground">
                  Duration: {(progress.result.duration / 1000).toFixed(2)}s
                </div>
              )}
            </div>
          )}
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto text-xs">
                  ...
                  {progress.result.duplicateEmails.length > 5 && (
                    <div className="text-blue-700 dark:text-blue-300 italic">
                      and {progress.result.duplicateEmails.length - 5} more...
                    </div>
                  )}
                </div>
              </div>
            ...
            progress.result.skippedLeads.length > 0 && (
              <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950/20 p-4">
                <div className="text-sm font-medium text-amber-900 dark:text-amber-100 mb-2">
                  Skipped Leads (Duplicates) ({progress.result.skippedLeads.length})
                </div>
                <div className="text-xs text-amber-700 dark:text-amber-300 mb-2">
                  ℹ️ Lead was entirely skipped because the Website URL already exists in workspace
                </div>
                <div className="space-y-1 max-h-32 overflow-y-auto text-xs">
                  ...
                  {progress.result.skippedLeads.length > 5 && (
                    <div className="text-amber-700 dark:text-amber-300 italic">
                      and {progress.result.skippedLeads.length - 5} more...
                    </div>
                  )}
                </div>
              </div>
