...
export default function CustomerGroupsPage() {
  ...
  const [editingCustomerGroup, setEditingCustomerGroup] =
  ...
  const handleCreateCustomerGroup = async (customerGroupData: unknown) => {
    createCustomerGroup.mutate(
      customerGroupData as CreateCustomerGroupRequest,
      {
        onSuccess: () => {
  ...
  const toggleCustomerGroupSelection = useCallback(
  ...
  return (
    <div className="space-y-6 h-full overflow-y-auto">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="text-sm text-muted-foreground">
                <span className="font-medium">
                  {selectedCustomerGroups.length}개 선택됨
                </span>
              </div>
      ...
      <Dialog
        ...
        onOpenChange={() => setEditingCustomerGroup(null)}
      >
        <DialogContent className="max-w-3xl max-h-[90vh]">
          <DialogHeader className="pb-4 border-b">
            <DialogTitle className="text-xl font-semibold">
