...
export const addressBookApi = {
  ...
  deleteGroup: (groupId: string) =>
    apiFetch<{ success: true }>(`/api/v1/address-book/groups/${groupId}?userId=${TEMP_USER_ID}`, {
      method: "DELETE",
    }),
  ...
  deleteContact: (contactId: string) =>
    apiFetch<{ success: true }>(
      `/api/v1/address-book/contacts/${contactId}?userId=${TEMP_USER_ID}`,
      { method: "DELETE" }
    ),
}
